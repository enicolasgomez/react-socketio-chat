  ...
  server.on('connection', socket => {
    ...
    socket.on('new message', (text: string) => {
      ...
      if (text.startsWith('/me ')) {
        ...
        event = normalizeEvent(Chats.addMeMessage(socket.user, socket.room, text, Date.now() ));
      }
      else {
        event = normalizeEvent(Chats.addChatMessage(socket.user, socket.room, text, Date.now() ));
      }
    ...
    socket.on('change nick', (nick: string) => {
      ...
      const event: Shared.ChatEvent = normalizeEvent(Chats.addNameChangeMessage(socket.user, socket.room, nick, Date.now() ));
